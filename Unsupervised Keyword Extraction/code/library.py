...
def clean_text_simple(text, my_stopwords, punct, remove_stopwords=True, pos_filtering=True, stemming=True):
    ...
    if remove_stopwords:
        ...
        tokens = [w for w in tokens if not w in my_stopwords]
    if stemming:
...
def unweighted_k_core(g):
    ...
    while len(gg.vs)>0:        
        while[d for d in gg.strength() if d <= i]: # while there still a vertex with a degree less than i
            j = [k for k, d in enumerate(gg.strength()) if d <= i][0]
            cores_g[gg.vs[j]['name']] = i 
            gg.delete_vertices(j)
        i += 1
